...
//                   {isOnline && !isCloud && !isGroup && user.isFriend && (
...
const ConversationList = ({ friends, onSelectUser, selectedId, onlineUsers = [], onSendFriendRequest, myInfo, onOpenCreateGroup }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const myId = myInfo?.userId || myInfo?._id;
  const [actionRender, setActionRender] = useState('all');

  const renderList = () => {
    // Lọc danh sách trước khi map để code rõ ràng hơn
    const filteredFriends = friends?.filter(user => {
      if (actionRender === 'unread') {
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {filteredFriends.length > 0 ? (
          filteredFriends.map((user) => {
            ...
            console.log("Tin nhắn chưa đọc của " + user.username + ": " + unreadCount);
            ...
            const renderLastMessageText = () => {
              ...
              if (lastMsg.messageType === 'system') {
                if (senderId === currentMyId) return `Bạn ${lastMsg.text || ""}`;
                const senderName = lastMsg.sender?.username || "";
                return `${senderName} ${lastMsg.text || ""}`.trim();
              }
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-0.5">
                    <h3 className={`text-[14px] truncate ${isUnread ? 'font-bold text-black' : (isSelected ? 'font-bold text-gray-900' : 'text-gray-800 font-medium')}`}>
                      {user.name || user.username}
                    </h3>
                    <span className={`text-[10px] shrink-0 ${isUnread ? 'text-blue-600 font-bold' : 'text-gray-400'}`}>
                      {formatTime(lastMsg?.createdAt)}
                  ...
                  <div className="flex justify-between items-center">
                    <p className={`text-[12px] truncate pr-2 ${isUnread ? 'font-semibold text-gray-900' : 'text-gray-500'}`}>
                      {renderLastMessageText()}
                    </p>
                    {isUnread && (
                      <div className="bg-red-500 text-white text-[10px] font-bold min-w-[18px] h-[18px] flex items-center justify-center rounded-full px-1 shadow-sm shrink-0">
    ...
    <div className="w-80 bg-white border-r border-gray-200 flex flex-col h-full">
      ...
      <div className="p-4 bg-white sticky top-0 z-10">
        <div className="flex items-center gap-2 mb-2">
          ...
          <button onClick={() => onOpenCreateGroup()} className="p-1.5 hover:bg-gray-100 rounded-md transition-colors" title="Tạo nhóm">
            <Users size={20} className="text-gray-700" />
          </button>
        </div>
        ...
        <div className="flex gap-4 mt-4 text-[13px] font-medium text-gray-500 px-1 border-b border-gray-100">
          <button
            onClick={() => setActionRender('all')}
            className={`pb-2 border-b-2 transition-colors ${actionRender === 'all' ? 'border-[#0068ff] text-[#0068ff]' : 'border-transparent hover:text-gray-800'}`}
          >
            Tất cả
          </button>
          <button
            onClick={() => setActionRender('unread')}
            className={`pb-2 border-b-2 transition-colors ${actionRender === 'unread' ? 'border-[#0068ff] text-[#0068ff]' : 'border-transparent hover:text-gray-800'}`}
          >
            Chưa đọc
          </button>
        </div>
